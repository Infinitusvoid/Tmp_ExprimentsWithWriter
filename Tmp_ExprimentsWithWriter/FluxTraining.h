...
struct TrainFlags {
    ...
    // For cache robustness
    std::string images_dir;        // dataset images folder
    bool clear_te_cache = true;    // delete *_flux_te.npz before run
    bool clear_vae_cache = false;  // optional: delete *_vae_latents.npz too
    bool add_skip_cache_check = true; // add --skip_cache_check to CLI
};
...
static bool write_train_bat(const fs::path& bat_path, const TrainFlags& F) {
    Writer_::Writer::Vars V{
        ...
        {"CLEAR_TE",          F.clear_te_cache ? "1" : "0"},
        {"CLEAR_VAE",         F.clear_vae_cache ? "1" : "0"},
        {"SKIP_CACHE_CHECK",  F.add_skip_cache_check ? "--skip_cache_check" : ""}
    };
...
REM ====== optional cache cleanup (broken NPZ from interrupted runs) ======
if "${CLEAR_TE}"=="1" (
  echo [CLEAN] Removing stale TE caches in "${IMAGES_DIR}"
  del /f /q "${IMAGES_DIR}\*_flux_te.npz" >nul 2>nul
)
if "${CLEAR_VAE}"=="1" (
  echo [CLEAN] Removing stale VAE caches in "${IMAGES_DIR}"
  del /f /q "${IMAGES_DIR}\*_vae_latents.npz" >nul 2>nul
)
...
int main() {
    ...
    const std::string TRIGGER = "Tw4kBigU";                  // fallback class token
    ...
    F.images_dir = IMAGES_DIR;  // <-- so the batch knows where to clean
    F.clear_te_cache = true;        // delete stale *_flux_te.npz before run
    F.clear_vae_cache = false;       // set true if you also want to wipe VAE latents
    F.add_skip_cache_check = true;      // add --skip_cache_check
